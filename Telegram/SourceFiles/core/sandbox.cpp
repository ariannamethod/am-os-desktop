...
ushort _fromHex(QChar c) {
  return ((c.unicode() >= uchar('a')) ? (c.unicode() - uchar('a') + 10)
                                      : (c.unicode() - uchar('0'))) &
         0x000F;
}
  ...
  for (int i = 0, l = str.size(); i != l; ++i) {
    ...
    if (ch == QChar::fromLatin1('%') && i + 4 < l) {
      result.append(QChar(ushort(
          (_fromHex(str.at(i + 1)) << 12) | (_fromHex(str.at(i + 2)) << 8) |
...
Sandbox::Sandbox(int &argc, char **argv)
    : QApplication(argc, argv), _mainThreadId(QThread::currentThreadId()) {
  connect(this, &QGuiApplication::screenAdded, this,
          [=](QScreen *) { setupScreenScale(); });
  connect(this, &QGuiApplication::screenRemoved, this,
          [=](QScreen *) { setupScreenScale(); });
  connect(this, &QGuiApplication::primaryScreenChanged, this,
          [=](QScreen *) { setupScreenScale(); });
}
...
int Sandbox::start() {
...

  {
    ...
    if (!_lockFile->tryLock() && Launcher::Instance().customWorkingDir()) {
      // On Windows, QLockFile has problems detecting a stale lock
      // if the machine's hostname contains characters outside the US-ASCII
      // character set.
  ...
  connect(&_localSocket, &QLocalSocket::connected, [=] { socketConnected(); });
  connect(&_localSocket, &QLocalSocket::disconnected,
          [=] { socketDisconnected(); });
  connect(&_localSocket, &QLocalSocket::errorOccurred,
          [=](QLocalSocket::LocalSocketError error) { socketError(error); });
  connect(&_localSocket, &QLocalSocket::bytesWritten,
          [=](qint64 bytes) { socketWritten(bytes); });
  connect(&_localSocket, &QLocalSocket::readyRead, [=] { socketReading(); });
  ...
  connect(this, &QCoreApplication::aboutToQuit,
          [=] { customEnterFromEventLoop([&] { closeApplication(); }); });
...
void Sandbox::setupScreenScale() {
  ...
  if (Platform::IsMac()) {
    ...
    cSetScreenScale(std::clamp(screenScale, style::kScaleMin,
...
bool Sandbox::event(QEvent *e) {
  if (e->type() == QEvent::Quit) {
    ...
    postEvent(static_cast<DeadlockDetector::PingPongEvent *>(e)->sender(),
...
void Sandbox::socketConnected() {
  LOG(("Socket connected, this is not the first application instance, sending "
       "show command..."));
  _secondInstance = true;

  QString commands;
  const QStringList &lst(cSendPaths());
  for (QStringList::const_iterator i = lst.cbegin(), e = lst.cend(); i != e;
       ++i) {
    commands += u"SEND:"_q + _escapeTo7bit(*i) + ';';
  }
  if (qEnvironmentVariableIsSet("XDG_ACTIVATION_TOKEN")) {
    commands += u"XDG_ACTIVATION_TOKEN:"_q +
                _escapeTo7bit(qEnvironmentVariable("XDG_ACTIVATION_TOKEN")) +
                ';';
  }
...
void Sandbox::socketReading() {
  ...
  const auto m =
      QRegularExpression(u"RES:(\\d+)_(\\d+);"_q).match(_localSocketReadData);
  if (!m.hasMatch()) {
    return;
  }
...
void Sandbox::socketError(QLocalSocket::LocalSocketError e) {
  ...
  if (!_localServer.listen(_localServerName)) {
    LOG(("Failed to start listening to %1 server: %2")
            .arg(_localServerName, _localServer.errorString()));
    return Quit();
  }
#endif // !Q_OS_WINRT

  if (!Core::UpdaterDisabled() && !cNoStartUpdate() &&
...
void Sandbox::singleInstanceChecked() {
  ...
  v::match(
      ...
      [&](const QByteArray &crashdump) {
        ...
        auto window =
            new LastCrashedWindow(_lastCrashDump, [=] { launchApplication(); });
        window->proxyChanges() | rpl::start_with_next(
                                     [=](MTP::ProxyData &&proxy) {
...
void Sandbox::socketDisconnected() {
  if (_secondInstance) {
    DEBUG_LOG(("Sandbox Error: socket disconnected before command response "
               "received, quitting..."));
    return Quit();
  }
}
...
void Sandbox::newInstanceConnected() {
  ...
  for (auto client = _localServer.nextPendingConnection(); client;
       client = _localServer.nextPendingConnection()) {
    _localClients.push_back(LocalClient(client, QByteArray()));
    connect(client, &QLocalSocket::readyRead, [=] { readClients(); });
    connect(client, &QLocalSocket::disconnected, [=] { removeClients(); });
  }
}
...
void Sandbox::readClients() {
  ...
  QString startUrl;
  QStringList toSend;
  for (LocalClients::iterator i = _localClients.begin(),
                              e = _localClients.end();
       i != e; ++i) {
    ...
    if (i->second.size()) {
      ...
      for (int32 to = cmds.indexOf(QChar(';'), from); to >= from;
        ...
        if (cmd.startsWith(u"CMD:"_q)) {
          ...
          const auto response =
              u"RES:%1_%2;"_q.arg(processId).arg(windowId).toLatin1();
          ...
          qputenv(
              ...
              _escapeFrom7bit(cmds.mid(from + 21, to - from - 21)).toUtf8());
        } else if (cmd.startsWith(u"OPEN:"_q)) {
          startUrl =
              _escapeFrom7bit(cmds.mid(from + 5, to - from - 5)).mid(0, 8192);
          const auto activationRequired = StartUrlRequiresActivate(startUrl);
          const auto processId = QApplication::applicationPid();
          const auto windowId = activationRequired ? execExternal("show") : 0;
          const auto response =
              u"RES:%1_%2;"_q.arg(processId).arg(windowId).toLatin1();
          i->first->write(response.data(), response.size());
        } else {
          LOG(("Sandbox Error: unknown command %1 passed in local socket")
                  .arg(cmd.toString()));
        }
        from = to + 1;
      }
...
void Sandbox::refreshGlobalProxy() {
  const auto proxy = !Core::IsAppLaunched() ? _sandboxProxy
...
void Sandbox::postponeCall(FnMut<void()> &&callable) {
  ...
  _postponedCalls.push_back({_loopNestingLevel, std::move(callable)});
}

void Sandbox::incrementEventNestingLevel() { ++_eventNestingLevel; }
...
MTP::ProxyData Sandbox::sandboxProxy() const { return _sandboxProxy; }
