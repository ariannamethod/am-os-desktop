...
void Launcher::prepareSettings() {
  ...
  if (cExeName().isEmpty()) {
    LOG(("WARNING: Could not compute executable path, some features will be "
         "disabled."));
  }
...
void Launcher::processArguments() {
  ...
  cSetDebugMode(parseResult.contains("-debug"));
  cSetKeyFile(parseResult.value("-key", {})
                  .join(QString())
                  .toLower()
                  .replace(RegExp, {}));
  cSetLaunchMode(parseResult.contains("-autostart")     ? LaunchModeAutoStart
                 : parseResult.contains("-fixprevious") ? LaunchModeFixPrevious
                 : parseResult.contains("-cleanup")     ? LaunchModeCleanup
                                                        : LaunchModeNormal);
  cSetNoStartUpdate(parseResult.contains("-noupdate"));
  cSetStartToSettings(parseResult.contains("-tosettings"));
  cSetStartInTray(parseResult.contains("-startintray"));
  cSetQuit(parseResult.contains("-quit"));
  cSetSendPaths(parseResult.value("-sendpath", {}));
  ...
  cSetStartUrl(parseResult.value("--", {}).join(QString()));

  const auto scaleKey = parseResult.value("-scale", {});
  if (scaleKey.size() > 0) {
    using namespace style;
    const auto value = scaleKey[0].toInt();
    cSetConfigScale(((value < kScaleMin) || (value > kScaleMax)) ? kScaleAuto
                                                                 : value);
  }
}
