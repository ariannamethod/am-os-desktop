...
void ShareBotGame(not_null<UserData *> bot, not_null<Data::Thread *> thread,
  ...
  histories.sendPreparedMessage(
      ...
      Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
          ...
          MTPInputQuickReplyShortcut(), MTPlong()),
  ...
  if (const auto history = entry->asHistory()) {
    ...
    owner->session()
        ...
        .request(MTPchannels_UpdatePinnedForumTopic(
            topic->channel()->inputChannel, MTP_int(topic->rootId()),
            MTP_bool(isPinned)))
    ...
    owner->session()
        ...
        .request(MTPmessages_ToggleSavedDialogPin(
            MTP_flags(flags), MTP_inputDialogPeer(sublist->peer()->input)))
        .done([=] { owner->notifyPinnedDialogsOrderUpdated(); })
        .send();
    // if (isPinned) {
    //	controller->content()->dialogsToUp();
...
void Filler::addToggleTopicClosed() {
  ...
  _addAction(
      closed ? tr::lng_forum_topic_reopen(tr::now)
      ...
      [=] {
        if (const auto strong = weak.get()) {
          controller->showSection(
              Info::Stories::Make(channel, Info::Stories::Tab::Archive));
        }
      },
        ...
        [=] {
          ...
          if (const auto sublist = weak.get()) {
            ...
            controller->showInNewWindow(SeparateId(
                SeparateType::SavedSublist, peer->owner().history(peer)));
          }
        },
        &st::menuIconNewWindow);
    AddSeparatorAndShiftUp(_addAction);
...
void Filler::addDeleteChat() {
  ...
  _addAction({
      .text = (_peer->isUser() ? tr::lng_profile_delete_conversation(tr::now)
  ...
  const auto blockAction = _addAction(
      ...
      [=] {
        ...
        if (user->isBlocked()) {
          ...
          window->show(
              Box(PeerMenuBlockUserBox, window, user, v::null, v::null));
        }
      },
...
void Filler::addExportChat() {
  ...
  const auto peer = _peer;
  _addAction(
      tr::lng_profile_export_chat(tr::now), [=] { PeerMenuExportChat(peer); },
      &st::menuIconExport);
}
...
void Filler::addReport() {
  ...
  _addAction(
      ...
      [=] { ShowReportPeerBox(navigation, peer); }, &st::menuIconReport);
}
...
void Filler::addBotToGroup() {
  ...
  [[maybe_unused]] const auto lifetime =
      ...
      rpl::start_with_next([=](QString label) {
        if (!label.isEmpty()) {
          const auto controller = _controller;
          _addAction(
              label,
...
void Filler::addCreatePoll() {
  ...
  const auto sendMenuType =
      (_request.section == Section::Scheduled) ? SendMenu::Type::Disabled
      : (_request.section == Section::Replies) ? SendMenu::Type::SilentOnly
                                               : SendMenu::Type::Scheduled;
  const auto flag = PollData::Flags();
  const auto replyTo = _request.currentReplyTo;
  auto callback = [=] {
    PeerMenuCreatePoll(controller, peer, replyTo, flag, flag, source,
                       {sendMenuType});
  };
...
void Filler::addTTLSubmenu(bool addSeparator) {
  if (_topic) {
    const auto validator = TTLMenu::TTLValidator(_controller->uiShow(), _topic);
    if (!validator.can()) {
      return;
    }
    const auto text =
        tr::lng_manage_topic_ttl_menu(tr::now) +
...
void Filler::addGiftPremium() {
  const auto user = _peer->asUser();
  if (!user || user->isInaccessible() || user->isSelf() || user->isBot() ||
      user->isNotificationsUser() || !user->canReceiveGifts() ||
      user->isRepliesChat() || !user->session().premiumCanBuy()) {
    return;
  }
  ...
  _addAction(
      ...
      [=] { navigation->showGiftPremiumBox(user); }, &st::menuIconGiftPremium);
}
...
void PeerMenuDeleteTopic(not_null<Window::SessionNavigation *> navigation,
                         not_null<ChannelData *> channel, MsgId rootId) {
  const auto api = &channel->session().api();
  api->request(
         MTPchannels_DeleteTopicHistory(channel->inputChannel, MTP_int(rootId)))
...
void PeerMenuShareContactBox(not_null<Window::SessionNavigation *> navigation,
  ...
  auto callback = [=](not_null<Data::Thread *> thread) {
    ...
    navigation->parentController()->show(Ui::MakeConfirmBox({
        ...
        .confirmed =
            [weak, user, navigation](Fn<void()> &&close) {
              if (const auto strong = weak.get()) {
...
void PeerMenuCreatePoll(not_null<Window::SessionController *> controller,
                        ...
                        PollData::Flags chosen, PollData::Flags disabled,
  ...
  auto box = Box<CreatePollBox>(controller, chosen, disabled, sendType,
                                sendMenuDetails);
  const auto weak = Ui::MakeWeak(box.data());
  const auto lock = box->lifetime().make_state<bool>(false);
  box->submitRequests() |
      rpl::start_with_next(
          [=](const CreatePollBox::Result &result) {
            if (std::exchange(*lock, true)) {
              return;
            }
            auto action =
                Api::SendAction(peer->owner().history(peer), result.options);
            ...
            api->polls().create(result.poll, action,
...
void PeerMenuBlockUserBox(
  ...
  if (user) {
    box->addRow(object_ptr<Ui::FlatLabel>(
        ...
        tr::lng_blocked_list_confirm_text(
            lt_name, rpl::single(Ui::Text::Bold(name)), Ui::Text::WithEntities),
  ...
  const auto allFromUser =
      v::is<ClearReply>(suggestClear)
          ? box->addRow(object_ptr<Ui::Checkbox>(
                box,
                tr::lng_delete_all_from_user(tr::now, lt_user,
...
ShowForwardMessagesBox(std::shared_ptr<ChatHelpers::Show> show,
  ...
  public:
    ...
    void rowClicked(not_null<PeerListRow *> row) override final {
      ...
      if (showLockedError(row) || (count && row->peer()->isForum())) {
        ...
        _hasSelectedChanges.fire(delegate()->peerListSelectedRowsCount() > 0);
      }
    }
    ...
    rowContextMenu(QWidget *parent,
      ...
      if (!count && !row->peer()->isForum()) {
        ...
        menu->addAction(
            ...
            [=] {
              delegate()->peerListSetRowChecked(row, !row->checked());
              _hasSelectedChanges.fire(delegate()->peerListSelectedRowsCount() >
                                       0);
            },
            &st::menuIconSelect);
        return menu;
  ...
  { // Chosen a single.
    auto chosen = [show, draft = std::move(draft)](
      ...
      const auto id = SeparateId(
          (peer->isForum() ? SeparateType::Forum : SeparateType::Chat), thread);
  ...
  const auto send = ShareBox::DefaultForwardCallback(
      show, session->data().message(msgIds.front())->history(), msgIds);
  ...
  const auto submit = [=](Api::SendOptions options) {
    ...
    send(ranges::views::all(peers) |
             ...
             ranges::to_vector,
         comment->entity()->getTextWithAppliedMarkdown(), options,
          ...
          [=](bool shown) {
            ...
            if (shown) {
              const auto send = state->box->addButton(tr::lng_send_button(),
                                                      [=] { submit({}); });
              send->setAcceptBoth();
              send->clicks() | rpl::start_with_next(
...
ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
  ...
  return ShowForwardMessagesBox(navigation,
...
ShowDropMediaBox(not_null<Window::SessionNavigation *> navigation,
  ...
  auto chosen = [data = std::move(data), callback = std::move(successCallback),
                 weak, navigation](not_null<Data::ForumTopic *> topic) mutable {
  ...
  auto initBox = [=](not_null<PeerListBox *> box) {
    ...
    forum->destroyed() |
        rpl::start_with_next([=] { box->closeBox(); }, box->lifetime());
  };
  *weak = navigation->parentController()->show(Box<PeerListBox>(
...
ShowSendNowMessagesBox(not_null<Window::SessionNavigation *> navigation,
  ...
  const auto error = GetErrorTextForSending(history->peer, {.forward = &list});
  if (!error.isEmpty()) {
...
void MenuAddMarkAsReadAllChatsAction(
  ...
  auto callback = [=, owner = &controller->session().data()] {
    ...
    controller->show(Ui::MakeConfirmBox({tr::lng_context_mark_read_all_sure(),
...
bool FillVideoChatMenu(not_null<SessionController *> controller,
  ...
  const auto creator = peer->isChat() ? peer->asChat()->amCreator()
