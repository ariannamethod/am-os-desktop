...
void ShowAutoDeleteToast(std::shared_ptr<Ui::Show> show,
...
TTLValidator::TTLValidator(std::shared_ptr<Ui::Show> show,
                           not_null<PeerData *> peer)
    : _peer(peer), _show(std::move(show)) {}

TTLValidator::TTLValidator(std::shared_ptr<Ui::Show> show,
                           not_null<Data::ForumTopic *> topic)
    : _peer(topic->peer()), _topic(topic), _show(std::move(show)) {}
...
Args TTLValidator::createArgs() const {
  ...
  auto callback = [=](TimeId period, Fn<void()>) {
    ...
    state->savingRequestId =
        api.request(MTPmessages_SetHistoryTTL(peer->input, MTP_int(period)))
            .done([=](const MTPUpdates &result) {
              peer->session().api().applyUpdates(result);
              ShowAutoDeleteToast(show, peer);
              state->savingRequestId = 0;
            })
            .fail([=] { state->savingRequestId = 0; })
            .send();
    show->hideLayer();
  };
  auto about1 =
      ...
      : peer->isBroadcast() ? tr::lng_ttl_edit_about_channel()
                            : tr::lng_ttl_edit_about_group();
  auto about2 = tr::lng_ttl_edit_about2(
      lt_link, tr::lng_ttl_edit_about2_link() | rpl::map([=](const QString &s) {
                 return Ui::Text::Link(s, "tg://settings/auto_delete");
               }),
      Ui::Text::WithEntities);
  auto about =
      rpl::combine(std::move(about1), std::move(about2)) |
...
bool TTLValidator::can() const {
  if (_topic) {
    return _topic->canEdit();
  }
  return (_peer->isUser() && !_peer->isSelf() &&
          !_peer->isNotificationsUser() &&
          !_peer->asUser()->isInaccessible()) ||
         (_peer->isChat() && _peer->asChat()->canEditInformation() &&
          _peer->asChat()->amIn()) ||
         (_peer->isChannel() && _peer->asChannel()->canEditInformation() &&
          _peer->asChannel()->amIn());
}

void TTLValidator::showToast() const { ShowAutoDeleteToast(_show, _peer); }

const style::icon *TTLValidator::icon() const { return &st::menuIconTTL; }
