...
[[nodiscard]] int CurrentCount() {
  return std::clamp(Core::App().settings().notificationsCount(), 1,
...
void AddTypeButton(not_null<Ui::VerticalLayout *> container,
                   not_null<Window::SessionController *> controller,
                   Data::DefaultNotify type, Fn<void(Type)> showOther) {
  using Type = Data::DefaultNotify;
  auto label = [&] {
    switch (type) {
    case Type::User:
      return tr::lng_notification_private_chats();
    case Type::Group:
      return tr::lng_notification_groups();
    case Type::Broadcast:
      return tr::lng_notification_channels();
    }
  ...
  const auto button = AddButtonWithIcon(container, std::move(label),
                                        st::settingsNotificationType, {icon});
  ...
  auto status =
      rpl::combine(NotificationsEnabledForTypeValue(session, type),
                   rpl::single(type) |
  ...
  button->geometryValue() |
      rpl::start_with_next(
          [=](const QRect &r) {
            ...
            toggleButton->setGeometry(r.x() + r.width() - w, r.y(), w,
                                      r.height());
            separator->setGeometry(toggleButton->x() - st::lineWidth,
                                   r.y() + (r.height() - separatorHeight) / 2,
                                   st::lineWidth, separatorHeight);
          },
  ...
  toggleButton->sizeValue() |
      rpl::start_with_next(
          [=](const QSize &s) {
            checkWidget->moveToRight(st.toggleSkip,
            ...
            if (!count) {
              ...
              controller->show(Box([=](not_null<Ui::GenericBox *> box) {
                ...
                Ui::ConfirmBox(
                    box,
                    {
                        .text = phrase(lt_count, rpl::single(float64(count)),
...
void NotificationsCount::paintEvent(QPaintEvent *e) {
  ...
  p.fillRect(
      screenRect.x(), screenRect.y(), st::notificationsBoxScreenSize.width(),
  ...
  for (int corner = 0; corner != 4; ++corner) {
    ...
    if (corner == static_cast<int>(_chosenCorner)) {
      ...
      for (int i = 0; i != kMaxNotificationsCount; ++i) {
        ...
        p.drawPixmapLeft(sampleLeft, sampleTop, width(),
                         _notificationSampleSmall);
        sampleTop += (isTop ? 1 : -1) * (st::notificationSampleSize.height() +
                                         st::notificationsSampleMargin);
      }
      p.setOpacity(1.);
    } else {
...
void NotificationsCount::setCount(int count) {
  ...
  for (; _oldCount != count; _oldCount += indexDelta) {
    _sampleOpacities[_oldCount + animatedDelta].start(
        [this] { update(); }, from, to, st::notifyFastAnim);
  }
...
void NotificationsCount::prepareNotificationSampleSmall() {
  ...
  auto sampleImage = QImage(QSize(width, height) * style::DevicePixelRatio(),
  ...
  sampleImage.fill(st::notificationBg->c);
  {
    ...
    p.drawEllipse(
        style::rtlrect(padding, padding, userpicSize, userpicSize, width));
    ...
    p.fillRect(style::rtlrect(closeLeft, padding, padding, padding, width),
               st::notificationSampleCloseFg);
  }
...
void NotificationsCount::prepareNotificationSampleUserpic() {
  if (_notificationSampleUserpic.isNull()) {
    _notificationSampleUserpic =
        Ui::PixmapFromImage(Window::LogoNoMargin().scaled(
...
void NotificationsCount::prepareNotificationSampleLarge() {
  ...
  auto sampleImage =
      QImage(w * style::DevicePixelRatio(), h * style::DevicePixelRatio(),
             QImage::Format_ARGB32_Premultiplied);
  sampleImage.setDevicePixelRatio(style::DevicePixelRatio());
  sampleImage.fill(st::notificationBg->c);
  {
    Painter p(&sampleImage);
    p.fillRect(0, 0, w - st::notifyBorderWidth, st::notifyBorderWidth,
               st::notifyBorder->b);
    p.fillRect(w - st::notifyBorderWidth, 0, st::notifyBorderWidth,
               h - st::notifyBorderWidth, st::notifyBorder->b);
    p.fillRect(st::notifyBorderWidth, h - st::notifyBorderWidth,
               w - st::notifyBorderWidth, st::notifyBorderWidth,
               st::notifyBorder->b);
    p.fillRect(0, st::notifyBorderWidth, st::notifyBorderWidth,
               h - st::notifyBorderWidth, st::notifyBorder->b);

    prepareNotificationSampleUserpic();
    p.drawPixmap(st::notifyPhotoPos.x(), st::notifyPhotoPos.y(),
                 _notificationSampleUserpic);

    int itemWidth = w - st::notifyPhotoPos.x() - st::notifyPhotoSize -
                    st::notifyTextLeft - st::notifyClosePos.x() -
                    st::notifyClose.width;

    auto rectForName = style::rtlrect(
        st::notifyPhotoPos.x() + st::notifyPhotoSize + st::notifyTextLeft,
        st::notifyTextTop, itemWidth, st::msgNameFont->height, w);

    auto notifyText = st::dialogsTextFont->elided(
        tr::lng_notification_sample(tr::now), itemWidth);
    p.setFont(st::dialogsTextFont);
    p.setPen(st::dialogsTextFgService);
    p.drawText(st::notifyPhotoPos.x() + st::notifyPhotoSize +
                   st::notifyTextLeft,
    ...
    auto notifyTitle =
        st::msgNameFont->elided(u"Telegram Desktop"_q, rectForName.width());
    p.drawText(rectForName.left(), rectForName.top() + st::msgNameFont->ascent,
               notifyTitle);
...
void NotificationsCount::mouseMoveEvent(QMouseEvent *e) {
  ...
  auto topLeft = style::rtlrect(screenRect.x(), screenRect.y(), cornerWidth,
                                cornerHeight, width());
  auto topRight =
      style::rtlrect(screenRect.x() + screenRect.width() - cornerWidth,
                     screenRect.y(), cornerWidth, cornerHeight, width());
  auto bottomRight =
      style::rtlrect(screenRect.x() + screenRect.width() - cornerWidth,
                     screenRect.y() + screenRect.height() - cornerHeight,
                     cornerWidth, cornerHeight, width());
  auto bottomLeft = style::rtlrect(
      screenRect.x(), screenRect.y() + screenRect.height() - cornerHeight,
      cornerWidth, cornerHeight, width());
  ...
  if (samplesNeeded > samplesLeave) {
    ...
    auto sampleLeft =
        (isLeft == rtl())
            ...
            : (r.x() + st::notifyDeltaX);
    auto sampleTop =
        isTop ? (r.y() + st::notifyDeltaY)
              : (r.y() + r.height() - st::notifyDeltaY - st::notifyMinHeight);
    for (int i = samplesLeave; i != samplesNeeded; ++i) {
      auto widget =
          std::make_unique<SampleWidget>(this, _notificationSampleLarge);
      widget->move(sampleLeft,
                   sampleTop + (isTop ? 1 : -1) * i *
                                   (st::notifyMinHeight + st::notifyDeltaY));
      widget->showFast();
...
void NotificationsCount::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (isDownCorner && _isOverCorner && _downCorner == _overCorner &&
...
NotificationsCount::SampleWidget::SampleWidget(NotificationsCount *owner,
                                               const QPixmap &cache)
    : _owner(owner), _cache(cache) {
  setFixedSize(cache.width() / cache.devicePixelRatio(),
...
void NotificationsCount::SampleWidget::startAnimation() {
  _opacity.start([=] { animationCallback(); }, _hiding ? 1. : 0.,
                 _hiding ? 0. : 1., st::notifyFastAnim);
}
...
NotifyPreview::NotifyPreview(bool nameShown, bool previewShown)
    : _nameShown(nameShown), _previewShown(previewShown),
      _roundRect(st::boxRadius, st::msgInBg),
      _userpic(u":/gui/icons/settings/dino.svg"_q),
      _logo(Window::LogoNoMargin()) {
  const auto ratio = style::DevicePixelRatio();
  _logo = _logo.scaledToWidth(st::notifyPreviewUserpicSize * ratio,
                              Qt::SmoothTransformation);
  _logo.setDevicePixelRatio(ratio);

  _name.setText(st::defaultSubsectionTitle.style,
...
int NotifyPreview::resizeGetHeight(int newWidth) {
  ...
  _height = st::notifyPreviewUserpicPosition.y() +
            st::notifyPreviewUserpicSize + st::notifyPreviewUserpicPosition.y();
  const auto available = _width - st::notifyPreviewTextPosition.x() -
...
void NotifyPreview::paint(Painter &p, int x, int y) {
  ...
  const auto userpic =
      QRect(st::notifyPreviewUserpicPosition,
  ...
  title.drawElided(p, st::notifyPreviewTitlePosition.x(),
  ...
  text.drawElided(p, st::notifyPreviewTextPosition.x(),
                  st::notifyPreviewTextPosition.y(),
                  _width - st::notifyPreviewTextPosition.x() - userpic.x(), 2);
}
...
SetupNotifyViewOptions(not_null<Window::SessionController *> controller,
                       not_null<Ui::VerticalLayout *> container, bool nameShown,
                       bool previewShown) {
  ...
  const auto makeCheckbox = [&](const QString &text, bool checked) {
    return Ui::MakeChatServiceCheckbox(widget, text, st::backgroundCheckbox,
                                       st::backgroundCheck, checked)
        .release();
  };
  const auto name =
      makeCheckbox(tr::lng_notification_show_name(tr::now), nameShown);
  const auto preview =
      makeCheckbox(tr::lng_notification_show_text(tr::now), previewShown);

  const auto view =
      widget->lifetime().make_state<NotifyPreview>(nameShown, previewShown);
      ...
      rpl::start_with_next(
          [=](int width) {
            ...
            const auto checksTop =
                height - (margins.bottom() + name->height()) / 2;
  ...
  widget->paintRequest() |
      rpl::start_with_next(
          [=](QRect rect) {
            ...
            Painter p(widget);
            view->paint(p, st::notifyPreviewMargins.left(),
                        st::notifyPreviewMargins.top());
          },
          widget->lifetime());

  name->checkedChanges() | rpl::start_with_next(
                               [=](bool checked) {
  ...
  preview->checkedChanges() | rpl::start_with_next(
                                  [=](bool checked) {
...
void SetupAdvancedNotifications(
  ...
  const auto position =
      container->add(object_ptr<NotificationsCount>(container, controller));
  ...
  count->sectionActivated() |
      rpl::start_with_next(
          [=](int section) { position->setCount(section + 1); },
          count->lifetime());
  Ui::AddSkip(container, st::settingsCheckboxesSkip);
}
...
void SetupMultiAccountNotifications(
  ...
  const auto fromAll =
      container
          ->add(object_ptr<Button>(container, tr::lng_settings_notify_all(),
                                   st::settingsButtonNoIcon))
          ->toggleOn(rpl::single(Core::App().settings().notifyFromAll()));
  fromAll->toggledChanges() | rpl::filter([](bool checked) {
...
void SetupNotificationsContent(not_null<Window::SessionController *> controller,
  ...
  const auto session = &controller->session();
  const auto checkbox = [&](rpl::producer<QString> label,
                            IconDescriptor &&descriptor,
                            rpl::producer<bool> checked) {
    auto result = CreateButtonWithIcon(
        container, std::move(label), st::settingsButton, std::move(descriptor));
    result->toggleOn(std::move(checked));
    return result;
  };
  const auto addCheckbox = [&](rpl::producer<QString> label,
                               IconDescriptor &&descriptor,
                               rpl::producer<bool> checked) {
    return container->add(
        checkbox(std::move(label), std::move(descriptor), std::move(checked)));
  };
  const auto &settings = Core::App().settings();
  const auto desktopToggles =
      container->lifetime().make_state<rpl::event_stream<bool>>();
  const auto desktop = addCheckbox(
      tr::lng_settings_desktop_notify(), {&st::menuIconNotifications},
      desktopToggles->events_starting_with(settings.desktopNotify()));

  const auto flashbounceToggles =
      container->lifetime().make_state<rpl::event_stream<bool>>();
  const auto flashbounce = addCheckbox(
      (Platform::IsWindows() ? tr::lng_settings_alert_windows
  ...
  const auto soundAllowed =
      container->lifetime().make_state<rpl::event_stream<bool>>();
  const auto allowed = [=] { return Core::App().settings().soundNotify(); };
  const auto sound =
      addCheckbox(tr::lng_settings_sound_allowed(), {&st::menuIconUnmute},
                  soundAllowed->events_starting_with(allowed()));

  const auto autoScroll = addCheckbox(
      tr::lng_settings_auto_scroll_inactive_chat(), IconDescriptor{},
      rpl::single(
          base::options::lookup<bool>(kOptionAutoScrollInactiveChat).value()));

  Ui::AddSkip(container);

  const auto checkboxes = SetupNotifyViewOptions(
      controller, container, (settings.notifyView() <= NotifyView::ShowName),
  ...
  const auto previewDivider =
      container->add(object_ptr<Ui::SlideWrap<Ui::BoxContentDivider>>(
          container, object_ptr<Ui::BoxContentDivider>(container)));
  ...
  auto joinSilent =
      rpl::single(session->api().contactSignupSilentCurrent().value_or(false)) |
      rpl::then(session->api().contactSignupSilent());
  const auto joined =
      addCheckbox(tr::lng_settings_events_joined(), {&st::menuIconInvite},
                  std::move(joinSilent) | rpl::map(!_1));
  joined->toggledChanges() | rpl::filter([=](bool enabled) {
  ...
  const auto pinned =
      addCheckbox(tr::lng_settings_events_pinned(), {&st::menuIconPin},
                  rpl::single(settings.notifyAboutPinned()) |
                      rpl::then(settings.notifyAboutPinnedChanges()));
  pinned->toggledChanges() | rpl::filter([=](bool notify) {
  ...
  const auto acceptCalls = addCheckbox(
      tr::lng_settings_call_accept_calls(), {&st::menuIconCallsReceive},
      authorizations->callsDisabledHereValue() | rpl::map(!_1));
  acceptCalls->toggledChanges() | rpl::filter([=](bool toggled) {
  ...
  const auto muted = container->add(object_ptr<Button>(
      container, tr::lng_settings_include_muted(), st::settingsButtonNoIcon));
  muted->toggleOn(rpl::single(settings.includeMutedCounter()));
  const auto count = container->add(object_ptr<Button>(
      container, tr::lng_settings_count_unread(), st::settingsButtonNoIcon));
  ...
  const auto native = [&]() -> Ui::SettingsButton * {
    ...
    return container
        ->add(object_ptr<Button>(container, std::move(nativeText),
                                 st::settingsButtonNoIcon))
        ->toggleOn(rpl::single(settings.nativeNotifications()));
  }();

  const auto advancedSlide =
      !Platform::Notifications::Enforced()
  ...
  desktop->toggledChanges() | rpl::filter([](bool checked) {
  ...
  sound->toggledChanges() | rpl::filter([](bool checked) {
  ...
  name->checkedChanges() | rpl::map([=](bool checked) {
    if (!checked) {
  ...
  preview->checkedChanges() | rpl::map([=](bool checked) {
    if (checked) {
  ...
  flashbounce->toggledChanges() | rpl::filter([](bool checked) {
  ...
  autoScroll->toggledChanges() | rpl::filter([](bool checked) {
    return (checked !=
            base::options::lookup<bool>(kOptionAutoScrollInactiveChat).value());
  }) |
      rpl::start_with_next(
          [=](bool checked) {
            base::options::set(kOptionAutoScrollInactiveChat, checked);
            Core::App().saveSettingsDelayed();
          },
          autoScroll->lifetime());

  muted->toggledChanges() | rpl::filter([=](bool checked) {
  ...
  count->toggledChanges() | rpl::filter([=](bool checked) {
      ...
      rpl::start_with_next(
          [=](Change change) {
            if (change == Change::DesktopEnabled) {
              ...
              previewWrap->toggle(Core::App().settings().desktopNotify(),
                                  anim::type::normal);
              previewDivider->toggle(!Core::App().settings().desktopNotify(),
                                     anim::type::normal);
  ...
  if (native) {
    native->toggledChanges() | rpl::filter([](bool checked) {
